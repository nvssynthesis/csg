...
//    float fedback_delta = dc_b.filter(zLine.getSample(0, rp));
    ...
    feedback = FM_filter.tpt_lp(fedback_delta * (fs * 0.000021f) * feedback_amt, feedback_cutoff);
    ...
    sinwin = PM_filter.tpt_lp(crush<float>(weighted_sincos, clamp<float>(bits2 + (_bits_B_MOD * bits2), 1.01f, 2048.f) ), PM_smooth);
